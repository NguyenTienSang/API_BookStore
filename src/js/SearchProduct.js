...
function fetchListCategoryBook() {
    ...
    .then(book => {
        ...
        nodeLinkLine.innerHTML = '<i class="fas fa-chevron-right"></i> ' + localStorage.getItem('TextTyping');
...
async function SearchingProduct() {
    fetchListCategoryBook();  
    var TextTyping = localStorage.getItem('TextTyping');
    ...
    .then(book => {
        var htmlProductType = '';
        for(var i=0; i<book.length; i++) {
            if(((book[i].Title).toLocaleLowerCase()).includes(TextTyping.toLocaleLowerCase())) {
